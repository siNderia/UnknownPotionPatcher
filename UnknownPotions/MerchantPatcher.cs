...
using static Mutagen.Bethesda.Skyrim.SoulGem;
    ...
    public static class MerchantPatcher
    {
        ...
        private static ILeveledItemGetter CloneLeveledList(ILeveledItemGetter source)
        {
            ...
            foreach (var entry in newList.Entries.EmptyIfNull())
            {
                ...
                if (bannedLists.Any(s => resolved.EditorID.Contains(s)))
                {
                    if (_logging) Console.WriteLine($"Skipping banned list: {resolved.EditorID}");
                    continue;
                }
